...
const localizeField = (valueByLanguage, language, fallbackValue) => {
  if (!valueByLanguage) {
    return fallbackValue;
  }

  return valueByLanguage[language] ?? valueByLanguage[DEFAULT_LANGUAGE] ?? fallbackValue;
};

const localizeSection = (section, language) => ({
  id: section.id,
  number: section.number,
  title: localizeField(section.title, language, ""),
  paragraphs: localizeField(section.paragraphs, language, []),
  trailingParagraphs: localizeField(section.trailingParagraphs, language, []),
  bullets: localizeField(section.bullets, language, []),
  bulletGroups: localizeField(section.bulletGroups, language, []),
  callout: localizeField(section.callout, language, null),
  keyValues: localizeField(section.keyValues, language, []),
  emailLine: localizeField(section.emailLine, language, null),
  contactDetails: localizeField(section.contactDetails, language, []),
});

export const getPrivacyContent = (language) => {
  const page = privacyPage[language] || privacyPage[DEFAULT_LANGUAGE];
